...
function entryHtml(r: StoredRound): string {
  ...
  return `<article class="card entry" data-id="${r.id}">
    <h2>${esc(r.name)}</h2>
    <p class="meta">${r.round === 'short' ? 'Short round' : 'Long round'} · ${r.mode === 'run' ? 'Run' : 'Walk'} · ${formatDuration(r.secs)} · ${formatDate(r.date)}${r.km !== null ? ` · ${r.km} km` : ''} · submitted ${new Date(r.createdAt).toLocaleString('en-GB')}</p>
